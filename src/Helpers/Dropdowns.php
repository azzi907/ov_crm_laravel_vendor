...
class Dropdowns
{
    ...
    public static function getUaeLocations($search = '')
    {
        self::initialize();
        $return_data = [];
        $file = Functions::directory() . 'locations_uae.json';
    ...
    public static function CommercialType($params = [])
    {
        ...
        $file = Functions::directory() . 'Commercial_types_'.'.json';
        $post_data = ["query" => (object) $query, "options" => $options];
    ...
    */
    ...
    public static function html_select_search($data, $options = [])
    {
        return View::make('optima::components.select-dropdown-search', compact('data', 'options'))->render();
    }
}