...
class CommercialProperties
{
    ...
    public static function findAll($page = 1, $page_size = 10, $query = '', $sort = ['current_price' => '-1'], $set_options = [])
    {
        ...
        if (isset($get['favorite_ids']) && !empty($get['favorite_ids'])) {
            ...
            $query_array["reference"] = [
                '$in' => is_array($get['favorite_ids']) ? $get['favorite_ids'] : [$get['favorite_ids']],
            ];
    ...
    public static function formateProperty($property, $set_options = [])
    {
        ...
        if (isset($property['listing_agent_data'])) {
            $f_property['agent_details'] = $property['listing_agent_data'];
        }
        ...
        if (isset($property['agency_data']['logo']['name']) && !empty($property['agency_data']['logo']['name'])) {
            $f_property['agency_logo'] = 'https://crm.optimaventurez.com/images/agencies/' . (isset($property['agency_data']['_id']) ? $property['agency_data']['_id'] : '') . '/' . (isset($property['agency_data']['logo']['name']) ? $property['agency_data']['logo']['name'] : '');
        }

        if (isset($property['listing_agency_data']['logo']['name']) && !empty($property['listing_agency_data']['logo']['name'])) {
            $f_property['agency_logo'] = 'https://crm.optimaventurez.com/images/companies/' . (isset($property['listing_agency_data']['_id']) ? $property['listing_agency_data']['_id'] : '') . '/' . (isset($property['listing_agency_data']['logo']['name']) ? $property['listing_agency_data']['logo']['name'] : '');
        ...
        if (isset($property['community'])) {
            $f_property['community_key'] = $property['community'];
        }
        if (isset($property['sub_community'])) {
            $f_property['sub_community_key'] = $property['sub_community'];
        }
        if (isset($property['property_community']['value'][$contentLang])) {
            $f_property['community'] = $property['property_community']['value'][$contentLang];
        } else if (isset($property['property_community']['value']['en'])) {
            $f_property['community'] = $property['property_community']['value']['en'];
        }
        if (isset($property['property_sub_community']['value'][$contentLang])) {
            $f_property['sub_community'] = $property['property_sub_community']['value'][$contentLang];
        } else if (isset($property['property_sub_community']['value']['en'])) {
            $f_property['sub_community'] = $property['property_sub_community']['value']['en'];
        }
        ...
        if (isset($property['property_attachments']) && count($property['property_attachments']) > 0 && !isset($property['from_residential'])) {
            ...
            foreach ($property['property_attachments'] as $pic) {
                if (isset($pic["publish_status"]) && !empty($pic["publish_status"])) {
                    ...
                    //     $attachments[] = self::$com_img . '/' . $pic['model_id'] . '/' . $set_options['image_size'] . '/' .  urldecode($pic['file_md5_name']);
                    // } 
        ...
        if (isset($property['property_size']) && $property['property_size'] != '') {
            $f_property['property_size'] = $property['property_size'];
        }
        ...
        if (isset($property['general_amenities']) && count($property['general_amenities']) > 0) {
            foreach ($property['general_amenities'] as $key => $value) {
                if ($value == true) {
                    $general_amenities[] = $key;
                }
            }
        }
        ...
        $f_property['property_features']['general_amenities'] = $general_amenities;
