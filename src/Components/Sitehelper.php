...
use Og\OptimaClass\Helpers\Dropdowns;
...
class Sitehelper
{
    ...
    public static function get_site_logo($object)
    {
        ...
        return isset($settings['header']['logo']['name']) ? ('https://crm.optimaventurez.com/images/cms_settings/' . self::$template . '/' . $settings['header']['logo']['name']) : '';
    }
    ...
    public static function uae_locations_available_html($post = [], $options = [])
    {
        $locations = Dropdowns::getUaeLocations();
        print_r($locations);die;
        
        echo self::html_select_2(
            collect($locations)->map(function ($location) {
                return [
                    'option_key' => $location['key'] ?? '',
                    'option_value' => $location['value'][App::getLocale()] ?? $location['value']['en'] ?? '',
                ];
            })->toArray(),
            [
                'id' => 'uae_locations', 
                'name' => 'uae_location[]',
                'multiple' => 'multiple',
                'placeholder' => Translate::t('location'),
                'class' => "ajaxmultiselect multiselect select2"
            ]
        );
